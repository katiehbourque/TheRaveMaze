...
import edu.wm.cs.cs301.abigaildanielandkatiebourque.generation.BSPBranch;
import edu.wm.cs.cs301.abigaildanielandkatiebourque.generation.BSPLeaf;
import edu.wm.cs.cs301.abigaildanielandkatiebourque.generation.BSPNode;
...
import edu.wm.cs.cs301.abigaildanielandkatiebourque.generation.MazeContainer;
import edu.wm.cs.cs301.abigaildanielandkatiebourque.generation.Wall;
...
public class MazeFileReader {
     ...
     */
    public MazeFileReader(FileInputStream filename) {
        load(filename) ;
    }
     ...
     */
    private void load(FileInputStream filename)
    {
        try{
            ...
            for (int temp = 0 ; temp < nList.getLength() ; temp++) {
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    Log.v("reader width ", String.valueOf(width));
                    height = getElementIntValue("sizeY", eElement);
                    Log.v("reader height ", String.valueOf(height));
                    ...
                    Log.v("reader cells ", String.valueOf(cells.width) + String.valueOf(cells.height) + String.valueOf(cells));
                    // read array of distance values
                    readDistances(eElement);
                    // read start position
                    startx = getElementIntValue("startX", eElement);
                    Log.v("reader startx ", String.valueOf(startx));
                    starty = getElementIntValue("startY", eElement);
                    Log.v("reader starty ", String.valueOf(starty));
    ...
    private static Wall readWall(Element eElement, int number, int i) {
        ...
        int[] coll = new int[3];
        coll = MazePanel.getColorComponents(col);
        result.setColor(coll[0], coll[1], coll[2]);
    ...
    private void readDistances(Element eElement) {
        ...
        for ( int x = 0; x != width; x++) {
            for ( int y = 0; y != height; y++) {
                ...
                Log.v("reader dists " + String.valueOf(x) + " "+ String.valueOf(y), String.valueOf(dists[x][y]));
                number++;
            }
        }
    }
